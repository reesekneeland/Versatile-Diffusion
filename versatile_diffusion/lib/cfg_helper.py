...
import importlib.resources as pkg_resources  # Keep for internal resources
...
def cfg_solve(cfg, cfg_root):
    ...
    #   all surrogate input are settled.
    #   (i.e. SAME(***) ) 
...
class model_cfg_bank(object):
    ...
    def __call__(self, name):
        if name not in self.cfg_bank:
            ...
            # **Modified**: Reference the correct subpackage and only pass the file name
            subpackage = 'versatile_diffusion.configs.model'
            resource_name = osp.basename(cfg_path)
            with pkg_resources.open_text(subpackage, resource_name) as f:
    ...
    def get_yaml_path(self, name):
        if name.find('openai_unet') == 0:
            return osp.join(self.cfg_dir, 'openai_unet.yaml')
        elif (name.find('clip') == 0) or (name.find('openclip') == 0):
            return osp.join(self.cfg_dir, 'clip.yaml')
        elif name.find('vd') == 0:
            return osp.join(self.cfg_dir, 'vd.yaml')
        elif name.find('optimus') == 0:
            return osp.join(self.cfg_dir, 'optimus.yaml')
        elif name.find('autokl') == 0:
            return osp.join(self.cfg_dir, 'autokl.yaml')
        else:
            raise ValueError
...
class dataset_cfg_bank(object):
    ...
    def __call__(self, name):
        if name not in self.cfg_bank:
            cfg_path = self.get_yaml_path(name)
            # **Modified**: Reference the correct subpackage and only pass the file name
            subpackage = 'versatile_diffusion.configs.dataset'
            resource_name = osp.basename(cfg_path)
            with pkg_resources.open_text(subpackage, resource_name) as f:
                cfg_new = yaml.load(f, Loader=yaml.FullLoader)
            cfg_new = edict(cfg_new)
    ...
    def get_yaml_path(self, name):
        if name.find('laion2b') == 0:
            return osp.join(self.cfg_dir, 'laion2b.yaml')
        else:
            raise ValueError
...
class experiment_cfg_bank(object):
    ...
    def __call__(self, name):
        if name not in self.cfg_bank:
            cfg_path = self.get_yaml_path(name)
            # **Modified**: Reference the correct subpackage and only pass the file name
            subpackage = 'versatile_diffusion.configs.experiment'
            resource_name = osp.basename(cfg_path)
            with pkg_resources.open_text(subpackage, resource_name) as f:
    ...
    def get_yaml_path(self, name):
        return osp.join(self.cfg_dir, name + '.yaml')

def load_cfg_yaml(path):
    # **Modified**: Determine the correct subpackage based on the path
    if osp.isfile(path):
        cfg_path = path
        # If it's an absolute or relative path, open normally
...
def cfg_initiates(cfg):
    ...
    if isresume:
        ...
        # **Modified**: Use standard open() for external files
        with open(resume_cfg_path, 'r', encoding='utf-8') as f:
        ...
        print('##########')
        print('')
        # **Modified**: Use standard open() for external log files
    ...
    if istrain:
        ...
        if osp.isdir(log_dir):
            if disable_log_override:
                raise ValueError(f'Override an existing log_dir is disabled at [{log_dir}]')
        else:
    ...
    if cfge.node_rank == 0:
        ...
        with open(log_file, 'a', encoding='utf-8') as f:
            pprint.pprint(cfg, f)
        # **Modified**: Use standard open() for external config.yaml
        with open(osp.join(log_dir, 'config.yaml'), 'w', encoding='utf-8') as f:
            yaml.dump(edict_2_dict(cfg), f)
    else:
        # **Modified**: Use standard open() for external config.yaml.{node_rank}
        with open(osp.join(log_dir, f'config.yaml.{cfge.node_rank}'), 'w', encoding='utf-8') as f:
            yaml.dump(edict_2_dict(cfg), f)
